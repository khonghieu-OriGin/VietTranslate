...
CREATE TABLE IF NOT EXISTS "user" (
    ...
    phone VARCHAR(20),
    role VARCHAR(20) NOT NULL,  -- 'hirer', 'translator', 'admin'
...
-- ─────────────────────────────────────────────────────────────────
-- 4. Service (Dịch vụ phiên dịch)
-- ─────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS service (
    id SERIAL PRIMARY KEY,
    profile_id INTEGER NOT NULL,
    service_type VARCHAR(50),  -- 'interpretation', 'translation', etc.
    description TEXT,
...
-- ─────────────────────────────────────────────────────────────────
-- 9. Direct Message (Tin nhắn trực tiếp)
-- ─────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────
-- 10. Deliverable (Sản phẩm giao dịch)
-- ─────────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS notification (
    ...
    message TEXT,
    notification_type VARCHAR(50),  -- 'job_update', 'message', 'review', etc.
    is_read BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES "user"(id) ON DELETE CASCADE
);
